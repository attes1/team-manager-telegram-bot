...
export const up = async <T>(db: Kysely<T>) => {
  // SQLite doesn't support adding columns with CHECK constraints
  // We need to recreate the table to add poll_cutoff_day and poll_cutoff_time
  ...
  // Copy existing data (new columns will use defaults)
  await sql`
...
export const down = async <T>(db: Kysely<T>) => {
  // Remove poll_cutoff_day and poll_cutoff_time columns
  await sql`
